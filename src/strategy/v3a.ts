...
import { BigNumberish, LIQUIDITY_STATE_LAYOUT_V4, Liquidity, LiquidityPoolKeys, LiquidityPoolKeysV4, LiquidityState, LiquidityStateV4, MARKET_STATE_LAYOUT_V3, Percent, TOKEN_PROGRAM_ID, Token, TokenAmount, getMultipleAccountsInfo, parseBigNumberish } from "@raydium-io/raydium-sdk";
...
import { ArbIdea, BotLiquidityState, LookupIndex, MintInfo, TxInstruction, TxPool } from "../types";
...
const processSwapBaseIn = async (signature: string, swapBaseIn: IxSwapBaseIn, instruction: TxInstruction, txPool: TxPool, ata: PublicKey, blockhash: string) => {
  ...
  // If token is registered as tracked token, update token balance in memory
  // BUG: Geyser missed payer transaction completed
  ...
  // The fastest way to know if this transaction is buy @ sell
  // Generate signer WSOL Token Account to be compare
  // and compare it with source Token Account, if it's similar then its buy
  // else it sell. If it did not match any of it, then skip this transaction
  // Originally, used `let account = await botTokenAccount.getTokenAccountInfo(sourceTA)`
  // but it's slow.
  let signerWSOLAccount = await BotTokenAccount.getAssociatedTokenAccount(new PublicKey(WSOL_ADDRESS), signer)

  // using findProgramAddressSync not always have a match
